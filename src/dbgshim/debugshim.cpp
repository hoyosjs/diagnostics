    ...
    CLR_DEBUGGING_PROCESS_FLAGS * pFlags)
{
    ...
    if ((ppProcess != NULL || pFlags != NULL) &&
        ...
        m_cdacLoadPolicy == CDacLoadPolicy_LegacyDacOnly)
    {
        // the library provider must be specified if either ppProcess or pFlags is non-NULL in the legacy path.
        hr = E_POINTER;
    }
    ...
    if (SUCCEEDED(hr) && (ppProcess != NULL || pFlags != NULL))
    {
        ...
        bool cdacEvaluated = m_cdacLoadPolicy != CDacLoadPolicy_LegacyDacOnly;
        if (cdacEvaluated)
        {
        ...
        else if (SUCCEEDED(fallbackHr))
        {
            hr = fallbackHr;
        }
        else if (m_cdacLoadPolicy == CDacLoadPolicy_CDacOnly ||
                 (cdacEvaluated && fallbackEvaluated))
        {
            // If cDAC was evaluated and failed, its error takes precedence if all other paths also failed.
            hr = cdacHr;
        }
        else
        {
...
static HRESULT ResolveLibraryPaths(
    ClrInfo& clrInfo,
    ...
    IUnknown** ppInstance)
{
    ...
    *ppInstance = NULL;
    ...
    if (m_cdacLoadPolicy == CDacLoadPolicy_CDacOnly ||
        (cdacEvaluated && fallbackEvaluated))
    {
        // Only replace a historical fallback error when both paths were evaluated.
        return cdacHr;
    }
    return fallbackHr;
}
